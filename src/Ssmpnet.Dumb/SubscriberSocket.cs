...
namespace Ssmpnet.Dumb
{
    public static class SubscriberSocket
    {
        ...
        public static void Start(IPEndPoint endPoint, Action<byte[],int,int> receiver, Action connected = null)
        {
            var socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);

            // XXX
            socket.SetSocketOption(SocketOptionLevel.Tcp, SocketOptionName.NoDelay, 1);
            socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReceiveBuffer, 1024 * 1024 * 1);
        ...
        private static void Retry(SubscriberToken st)
        {
            ...
            Action<byte[],int,int> receiver = st.Receiver;
        ...
        static void CompletedReceive(object sender, SocketAsyncEventArgs e)
        {
            ...
            if (e.SocketError == SocketError.Success)
            {
                st.Receiver(e.Buffer, 0, e.BytesTransferred);
